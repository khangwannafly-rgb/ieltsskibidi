        ...
        <AnimatePresence>
          {score && (
                ...
                <div className="p-8 border-t border-white/10 bg-white/[0.02] flex gap-4">
                  ...
                  >
                    Xem lại câu hỏi
                        ...
                        <span className="text-[10px] font-black text-indigo-600 dark:text-indigo-400 uppercase tracking-widest block mb-1">Band Score</span>
                        <span className="text-4xl font-black text-indigo-600 dark:text-indigo-400">{score.overallBand}</span>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {Object.entries(score.criteria || {}).map(([key, value]: [string, any]) => {
                        const criteriaMap: Record<string, string> = {
                          'fluency_and_coherence': 'Trôi chảy & Mạch lạc',
                          'lexical_resource': 'Vốn từ vựng',
                          'grammatical_range_and_accuracy': 'Ngữ pháp & Chính xác',
                          'pronunciation': 'Phát âm'
                        };
                        return (
                          <div key={key} className="bg-slate-50 dark:bg-slate-800/50 p-5 rounded-2xl border border-slate-100 dark:border-slate-700">
                            <div className="flex justify-between items-center mb-2">
                              <span className="text-xs font-black text-slate-600 dark:text-slate-400 uppercase tracking-wider">
                                {criteriaMap[key] || key}
                              </span>
                              <div className="flex items-baseline gap-1">
                                <span className="text-2xl font-black text-slate-900 dark:text-white">{value.score}</span>
                                <span className="text-[10px] font-bold text-slate-600 dark:text-slate-400">/9</span>
                              </div>
                            </div>
                            <p className="text-xs text-slate-600 dark:text-slate-400 leading-relaxed italic">"{value.feedback}"</p>
                          </div>
                        );
                      })}
                    </div>
                    ...
                    <div className="space-y-4">
                      <div className="bg-emerald-50 dark:bg-emerald-900/20 p-6 rounded-2xl border border-emerald-100 dark:border-emerald-900/30">
                        <h4 className="text-sm font-black text-emerald-700 dark:text-emerald-400 uppercase tracking-widest mb-2">Điểm mạnh</h4>
                      ...
                      <div className="bg-rose-50 dark:bg-rose-900/20 p-6 rounded-2xl border border-rose-100 dark:border-rose-900/30">
                        <h4 className="text-sm font-black text-rose-700 dark:text-rose-400 uppercase tracking-widest mb-2">Cần cải thiện</h4>
                        <ul className="space-y-2">
                          {score.improvements?.map((imp: string, i: number) => (
                            <li key={i} className="text-sm text-rose-800 dark:text-rose-300 flex items-start gap-2">
                    ...
                    <div className="flex flex-col gap-3">
                      ...
                      <button 
                        ...
                        className="w-full py-4 text-slate-600 dark:text-slate-400 font-black uppercase tracking-widest text-xs hover:text-slate-800 dark:hover:text-white transition-colors"
                      >
                        Luyện tập đề khác
                      </button>
                    </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="bg-indigo-600 dark:bg-indigo-700 p-8 rounded-[2.5rem] text-white space-y-4 shadow-2xl shadow-indigo-200 dark:shadow-none">
                <h4 className="font-black text-indigo-200 uppercase tracking-widest text-[10px]">Lời khuyên từ giám khảo</h4>
                <p className="text-lg font-bold leading-relaxed italic">
                  "Đừng lo lắng quá nhiều về ngữ pháp hoàn hảo. Hãy tập trung vào sự trôi chảy và mở rộng câu trả lời của bạn bằng các ví dụ."
                </p>
              </div>
              <div className="bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] border border-slate-200 dark:border-slate-800 space-y-4 shadow-sm">
                <h4 className="font-black text-slate-600 dark:text-slate-400 uppercase tracking-widest text-[10px]">Từ vựng hữu ích</h4>
                <div className="flex flex-wrap gap-2.5">
                  {score.suggestedVocabulary.map((w: string) => (
                    <span key={w} className="px-3 py-1.5 bg-slate-50 dark:bg-slate-800 text-slate-700 dark:text-slate-300 rounded-lg text-xs font-bold border border-slate-100 dark:border-slate-700">
                      {w}
                    </span>
                  ))}
                </div>
